...
void setup(void)
{
  ...
  Serial.begin(9600);
  Serial.println("Setup Started");//luke
...
void loop()
{
  ...
  */
  ...
  while (1)
  {
    if (buttonPressed == true && (digitalRead(BUTTON1) == LOW) ) {
      rainCounter++;
      delay(50);
      buttonPressed = false;
    }
    ...
    if (RTC_ALARM == true)
    {
      ...
      float sensor0P = 100.00 - ( ( 100.00 * moist ) / 1023.00 );
      uint8_t sensorInt = (uint8_t) sensor0P;
      Serial.print("Moisture : ");
      Serial.print(sensorInt);
      Serial.println("%");
      lora.TX.Data[0] = sensorInt;
      

      int16_t val;
      float rainValue = rainCounter * 0.15 * 100.0 ;
      Serial.println(rainValue);
      val = (int16_t) (rainValue); //Times 10.0 because of it automaticlly dived it by 10.
      Serial.println(val);
      lora.TX.Data[1] = lowByte(val) ;
      lora.TX.Data[2] = highByte(val);
      lora.TX.Count = 3;
...
void go_to_sleep (void)
{
  ...
  Serial.println("Going to sleep..."); //Luke
